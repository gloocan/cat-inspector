...
function splitFirstGenericArg(content: string): [string, string] | null {
	let depth = 0
 ...
 */
export function peelOuterLabeled(s: string): string | null {
	const t = s.trim()
...
export function peelReadonly(s: string): string | null {
	const t = s.trim()
...
export function peelPromise(s: string): string | null {
	const t = s.trim()
 ...
 */
export function normalizeReturnTypeForRpcCompare(input: string): string {
	let s = input.trim()
	let prev = ''
	while (s !== prev) {
		prev = s
 ...
 */
export function typesMatchForRpc(expected: string, actual: string): boolean {
