...
import type { RegistryEntry, RpcErrorDetail, RpcResponse } from './types.js'
...
async function withInvokeTimeout<T>(promise: Promise<T>, ms: number): Promise<T> {
...
function validateParamsSchemaIfConfigured(
	requestId: string,
	fnKey: string,
	entry: RegistryEntry,
	args: unknown[],
	...
	if (!ser.validateParamsJsonSchema || !entry.paramsJsonSchema) return null
...
function validateReturnSchemaIfConfigured(
	requestId: string,
	fnKey: string,
	entry: RegistryEntry,
	result: unknown,
	...
	const ser = getRpcSerializationConfig()
	if (!ser.enabled || !ser.validateReturnJsonSchema || !entry.returnJsonSchema) return null
...
async function executeHttpBridgeRequest(
	request: { requestId: string; fnKey: string; args: unknown[] },
	entry: RegistryEntry,
	start: number,
): Promise<RpcResponse> {
	...
	if (!spec) {
		return errResp(
			requestId,
			fnKey,
			'HTTP_BRIDGE_NOT_CONFIGURED',
			`call registerHttpBridgeRoute for ${fnKey}`,
			start,
			'validation',
			null,
			'HTTP_BRIDGE_NOT_CONFIGURED',
	...
	try {
		...
		let httpOut: { statusCode: number; headers: Record<string, string>; body: unknown }
		try {
			httpOut =
			...
			if (msg.includes('invokeTimeoutMs') || msg.includes('exceeded')) {
				...
				return errResp(
					...
					fnKey,
					'INVOKE_TIMEOUT',
					`handler exceeded invokeTimeoutMs (${invokeMs}ms)`,
					start,
					'validation',
					null,
					'INVOKE_TIMEOUT',
...
function buildOkRpcResponse(
	...
	if (artifacts) base.artifacts = artifacts
	return base
}
...
export async function executeRPC(request: {
	...
	if (!Registry.has(fnKey)) {
		return errResp(
			...
			fnKey,
			'FN_NOT_FOUND',
			`Not found. Available: ${[...Registry.keys()].join(', ')}`,
			start,
			'validation',
		)
	}
	...
	if (args.length !== expected) {
		return errResp(
			...
			fnKey,
			'WRONG_ARG_COUNT',
			`Expected ${expected} args, got ${args.length}`,
			start,
			'validation',
		)
	}
	...
	if (paramsSchemaHit) return paramsSchemaHit
	...
	const maybeFn =
		entry.style === 'function'
			? entry.originalFn
			: // eslint-disable-next-line @typescript-eslint/no-explicit-any
				((resolveInstanceForClassName(className) as any)?.[methodName] as unknown)
	...
	if (typeof maybeFn !== 'function') {
		...
		if (!resolveInstanceForClassName(className)) {
			return errResp(
				...
				fnKey,
				'NO_INSTANCE',
				`No instance for ${className}. Ensure the class method is decorated with @Cat (constructor auto-registered) or call registerInstance().`,
				start,
				'validation',
			)
		}
		return errResp(
			...
			fnKey,
			'NOT_A_FUNCTION',
			`${methodName} not callable on ${className}`,
			start,
			'validation',
		)
	}
	...
	try {
		...
		const invokePromise = maybeFn(...args) as Promise<unknown>
