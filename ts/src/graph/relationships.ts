...
function dedupePush(arr: string[], key: string): void {
	if (!arr.includes(key)) arr.push(key)
}

export function resolveRelationships(): void {
	const allKeys = [...Registry.keys()]

	Registry.forEach((meta, fnKey) => {
		for (const otherKey of allKeys) {
			if (otherKey === fnKey) continue
...
export function analyzeRelationships(): {
	...
	Registry.forEach((meta, key) => {
		...
		if (!hasParents && hasChildren) roots.push(key)
		if (hasParents && !hasChildren) leaves.push(key)
		if (hasParents && hasChildren) middle.push(key)
	})

	return { roots, leaves, middle }
}

export function buildTree(fnKey: string, visited = new Set<string>()): object {
	if (visited.has(fnKey)) return { key: fnKey, circular: true }
	visited.add(fnKey)

	const meta = Registry.get(fnKey)
	if (!meta) return { key: fnKey, error: 'not found' }
...
export function groupApiPipelines(): Record<string, string[]> {
	const grouped: Record<string, string[]> = {}
	Registry.forEach((meta, fnKey) => {
		if (meta.pipelineId === null || meta.pipelineIndex === null) return
		if (!grouped[meta.pipelineId]) grouped[meta.pipelineId] = []
