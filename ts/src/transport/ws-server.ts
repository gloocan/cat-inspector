...
import type { RegistryEntry, RpcResponse } from '../types.js'
...
function rpcRateLimitedResponse(requestId: string, fnKey: string, retryAfterMs: number): RpcResponse {
	return {
		type: 'RPC_RESPONSE',
		requestId,
		fnKey,
...
function rpcUploadMaterializeError(requestId: string, fnKey: string, message: string, start: number): RpcResponse {
	return {
		type: 'RPC_RESPONSE',
		requestId,
		fnKey,
...
function getAuthTokenFromRequest(req: IncomingMessage): string | undefined {
	const url = req.url
...
export function startInspectorWebSocket(
	registrySnapshot: Record<string, RegistryEntry>,
	tree: object[],
	options: InspectorWebSocketOptions,
): Promise<InspectorWebSocketHandle> {
	...
	return new Promise((resolve, reject) => {
		...
		wss.on('connection', (ws: WebSocket, req: IncomingMessage) => {
			...
			const redactedRegistry = Object.fromEntries(
				Object.entries(registrySnapshot).map(([k, v]) => [k, { ...v, body: '' }]),
			)
			...
			ws.on('message', async (data) => {
				...
				function sendUploadError(code: string, message: string, uploadId: string | null): void {
					ws.send(
						JSON.stringify({
				...
				if (m.type === 'QA_UPLOAD_START') {
					if (!uploadStore) {
						sendUploadError(
							'UPLOAD_DISABLED',
							'QA file uploads are not enabled on this WebSocket server',
							typeof m.uploadId === 'string' ? m.uploadId : null,
						)
						return
					}
					try {
						...
						const contentType =
							typeof m.contentType === 'string' ? m.contentType : 'application/octet-stream'
						const sizeBytes =
							typeof m.sizeBytes === 'number' && Number.isFinite(m.sizeBytes) ? m.sizeBytes : -1
						if (sizeBytes < 0) {
				...
				if (m.type === 'QA_UPLOAD_ABORT') {
					if (uploadStore) uploadStore.abort(connKey)
					ws.send(
						JSON.stringify({
							type: 'QA_UPLOAD_ACK',
							protocolVersion: PROTOCOL_VERSION,
				...
				if (m.type === 'RPC_CALL') {
					if (typeof m.requestId !== 'string' || typeof m.fnKey !== 'string') return
					if (!Array.isArray(m.args)) return

					const t0 = performance.now()
					const requestId = m.requestId as string
					const fnKey = m.fnKey as string
					let args = m.args as unknown[]

					const entry = Registry.get(fnKey)
					const fileUrlOpts = options.fileUrl
					const canMaterializeRefs = Boolean(uploadStore && entry)
					const canMaterializeUrls = Boolean(
						entry && fileUrlOpts && fileUrlOpts.allowedHosts && fileUrlOpts.allowedHosts.length > 0,
					)
					...
					if (!invokeRateLimitAllow(connKey)) {
						const resp = rpcRateLimitedResponse(
							requestId,
							fnKey,
							invokeRateLimitRetryAfterMs(connKey),
						)
						ws.send(JSON.stringify(resp))
						await invokeAudit({
							fnKey,
				...
				if (m.type === 'COVERAGE_REQUEST') {
					const roots: string[] = Array.isArray(options.scanRoots)
