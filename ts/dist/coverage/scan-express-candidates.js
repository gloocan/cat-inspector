...
function stringLiteralText(expr) {
    if (!expr)
        return null;
...
function normalizeHandlerExpression(expr) {
...
function exprDisplayName(source, expr) {
    ...
    if (ts.isCallExpression(normalized))
        return normalized.expression.getText(source);
    return normalized.getText(source);
}
function maybeResolveFnKeyFromExpression(expr) {
...
function routeInfoFromCall(methodName, pathExpr) {
...
function extractHandlersForExpressCall(calleeName, args) {
...
function extractHandlersForRegisterCatPipelineCall(args) {
    ...
    if (!methodExpr || !routeExpr || !handlersExpr)
        return null;
    ...
    if (!ts.isArrayLiteralExpression(handlersExpr))
        return { route, handlers: [] };
    const handlers = flattenHandlers(handlersExpr);
    return { route, handlers };
}
function tryExtractExpressRegistration(node) {
    ...
    if (ts.isIdentifier(node.expression) && node.expression.text === 'registerCatPipeline') {
        ...
        if (extracted)
            return { ...extracted, methodName: 'registerCatPipeline' };
    }
...
export function scanExpressCandidates(rootDir, options = {}) {
    ...
    function resolveHandlerDeclaration(expr) {
        const n = normalizeHandlerExpression(expr);
    ...
    function shouldKeepHandler(expr) {
                    ...
                    if (reg.route) {
                        ...
                        for (let i = 0; i < hs.length; i++) {
                            ...
                            if (kind === 'controller')
                                ...
                                middleware.push(base);
                            if (resolvedFnKey === null)
                                unresolvedHandlers++;
                        }
                    }
                    else {
...
//# sourceMappingURL=scan-express-candidates.js.map