...
function getFunctionLikeSignatureTypes(fn, checker, options) {
...
function isFileLikeType(t, checker) {
    const sym = t.getSymbol() ?? t.aliasSymbol;
    const name = sym?.getName();
    if (name && FILE_TYPE_NAMES.has(name))
        return true;
...
function computeFileMetaForType(t, checker, depth, 
...
seen) {
    ...
    const filePaths = [];
    const fileArrayPaths = [];
    const collect = (subType, prefix) => {
        ...
        if (typeof anyChecker.isArrayType === 'function' && anyChecker.isArrayType(subType)) {
            // eslint-disable-next-line @typescript-eslint/no-unsafe-call
            const el = anyChecker.getElementTypeOfArrayType(subType);
            if (el && isFileLikeType(el, checker)) {
            ...
            if (el) {
                ...
                for (const p of nested.filePaths ?? [])
                    filePaths.push(`${prefix}[0].${p}`);
                for (const p of nested.fileArrayPaths ?? [])
                    fileArrayPaths.push(`${prefix}[0].${p}`);
            }
            return;
        }
        const nested = computeFileMetaForType(subType, checker, depth + 1, seen);
        for (const p of nested.filePaths ?? [])
            filePaths.push(`${prefix}.${p}`);
        for (const p of nested.fileArrayPaths ?? [])
            fileArrayPaths.push(`${prefix}.${p}`);
    };
    ...
    const out = {};
    if (filePaths.length)
        out.filePaths = [...new Set(filePaths)];
    if (fileArrayPaths.length)
        out.fileArrayPaths = [...new Set(fileArrayPaths)];
    return out;
}
...
function parseFnKeyLiteral(fnKey) {
    const dot = fnKey.indexOf('.');
    if (dot <= 0 || dot === fnKey.length - 1)
        return undefined;
    return { className: fnKey.slice(0, dot), fnName: fnKey.slice(dot + 1) };
}
function tryVisitFunctionalRegistrations(node, checker, results, options) {
...
function getMethodSignatureTypes(member, checker, options) {
...
function hasCatDecorator(member) {
    ...
    return mods.some((mod) => {
        ...
        if (ts.isIdentifier(expr) && expr.text === 'Cat')
            return true;
...
export function visitNode(node, _source, checker, results, options) {
